...
grep -r -n -i -C ${contextSize} --color=always --exclude-dir sfdcantproject --exclude \*.layout --exclude \*_Test.cls --exclude \*.profile --exclude gbc_\*_js.js --exclude \*.resource --exclude \*.permissionset "${search}" . | aha --title "${search} used" --line-fix --black > "${filePath}"
open "${filePath}"