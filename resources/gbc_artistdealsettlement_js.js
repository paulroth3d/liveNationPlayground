...
/*
//Filename:grunt_src/artistdealsettlement.js
//-- #	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#
*/
convertGridInfoMap();
	...
	jq(allRows).each(function(i, e){
		...
		if(!oldVal.trim() || (reorder && oldVal != parentRowCount)){
			...
			try{
				...
				//gb throws an error when we manually invoke the change event on one of it's inputs, but it does not prevent saving...
			}
		}
	});
}
...
function changedDealType(){
	...
	if( dealType == 'Flat' || dealType == 'Retro Gross Deal' || dealType == 'Retro Net Deal' || dealType == 'Step Up Gross Deal' || dealType == 'Step Up Net Deal'){
		backendField.val('');
	...
	if( dealType == 'Alternative Promoter Profit Deal' || dealType == 'Gross Split Point' || dealType == 'Percentage of Gross' || dealType == 'Percentage of Net' || dealType == 'Promoter Profit'){
		if(backendField.val() == ''){
	...
	if(dealType == 'Gross Split Point' || dealType == 'Percentage of Gross' || dealType == 'Retro Gross Deal' || dealType == 'Step Up Gross Deal' ){
		dealCalculationField.val('Inclusive of Sound & Lights & Support & Other Talent Amount').change();
...
function changedTypeOnly(){
	...
	if(type == 'Co-Headliner') {
		disableBackendFields(jq(this.closest('tr')), false);
	}

	if (type == 'Support') {
		disableBackendFields(jq(this.closest('tr')), true);
	}
}
...
function disableBackendFields(parentRow, clearValues) {
	jq(parentRow).find(createFieldSelector('DealType__c')).click();
	jq(parentRow).find(createFieldSelector('DealCalculation__c')).click();

	var backendField = jq(parentRow).find(createFieldInputSelector('BackendPercent__c'));
	var dealTypeField = jq(parentRow).find(createFieldInputSelector('DealType__c'));
	var dealCalculationField = jq(parentRow).find(createFieldInputSelector('DealCalculation__c'));
...
function findPrimaryHeadliner() {
	...
	for(var i = 0; i < dealFields.length ; i++){
		...
		if(type == 'Co-Headliner') {
			disableBackendFields(jq(jq(dealFields[i]).closest('tr')), false);
		}

		if (type == 'Support') {
			disableBackendFields(jq(jq(dealFields[i]).closest('tr')), true);
		}
	}
}

function documentIsReady() {
	if( window.wasDocumentReadyCalled == true ){
		return;
	}
	
	window.wasDocumentReadyCalled = true;
    ...
    jq("#gbMainTable " + createFieldSelector("StageType__c")).hide();

    jq('.createNew').click(function() {
    	try {
	    	jq("#gbMainTable " + createFieldSelector("StageType__c") + " div").click();
	    } catch (e) {
	    	console.log(e);
	    }
	    try {
    		jq("#gbMainTable " + createFieldInputSelector("StageType__c")).val('Settlement');
    	} catch (e) {
	    	console.log(e);
	    }
    	try {
    		jq("#gbMainTable " + createFieldInputSelector("StageType__c")).change();
    	} catch (e) {
	    	console.log(e);
	    }
    	try {
	...
	jq('.groupByRow').each(function(i,l){
	  ...
	  if(isCoHeadliner){
	    ...
	    jq(l).parent().next().find('.co-headliner').find(dealApplicationType).find('select').attr('disabled','true')
...
function checkDocumentReady(){
	if( document.readyState !== 'complete' ){
		setTimeout( checkDocumentReady(), 1000 );
	} else {
		documentIsReady();
	}
}
...
window.wasDocumentReadyCalled = false;

jq( document ).ready( documentIsReady );
/*
//Filename:grunt_src/artistDealSettlementBonusDefinition.js
//-- #	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#
*/
...
/*
//Filename:grunt_src/artistRetroSettlementBonusDefinition.js
//-- #	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#	#
*/
...
jq(document).ready(function() {
    ...
    */
    ...
    var artistDealRetro = new lneBonusHelp(fieldInfoRetro);
